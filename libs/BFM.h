...
static double* convert_to_double(float* data, int size, bool isColor=false) {
    double* double_data = new double[size];
    for (int i = 0; i < size; i++) {
        if ( i >= 17067007 && isColor)
        {
          double_data[i] = 0.0;
        }
        else {
...
static BFM bfm_setup() {
    ...
    bfm.color_pca_basis = convert_to_double((float*)load_binary_data(color_pca_basis_path.c_str()), 85764*199, true);
